...
def generate_graph(num_nodes, sparsity, seed=42):
    ...
    Enforces u < v for all edges.
    """
    random.seed(seed)
    G = nx.DiGraph()
    G.add_nodes_from(range(num_nodes))
    
    # Add edges only if u < v
    for u in range(num_nodes):
        for v in range(num_nodes):
            if u < v and random.random() < sparsity:
                G.add_edge(u, v)
...
def main():
    ...
    parser.add_argument("--path_repeat", type=int, default=1, help="Number of times to repeat each path in training data")

    args = parser.parse_args()

    # Create a descriptive subfolder name
    # Create a descriptive subfolder name
    subfolder_name = f"dag_n{args.num_nodes}_p{args.sparsity}"
    ...
    with open(train_file, 'w') as f:
        for item in train_data:
            for _ in range(args.path_repeat):
                f.write(item['text'] + "\n")
